...
export function NewsPagination({ totalPages, currentPage }: Props) {
  ...
  return (
    <nav 
      aria-label="Навигация по новостной ленте" 
      className="flex justify-center items-center gap-1.5 sm:gap-2 mt-16 sm:mt-24 relative z-10 pointer-events-auto"
    >
      {/* Back Button */}
      {currentPage <= 1 ? (
        <span 
        ...
        {pages.map((p, idx) => {
          ...
          return (
            <Link
              ...
              className={`w-7 sm:w-8 md:w-10 h-7 sm:h-8 md:h-10 flex items-center justify-center rounded-full text-[10px] sm:text-xs md:text-sm font-bold transition-all duration-300 cursor-pointer focus-visible:ring-2 focus-visible:ring-amber-500 focus-visible:outline-hidden ${
                isCurrent
