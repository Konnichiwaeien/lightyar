...
export function CampaignShare({ title }: { title: string }) {
  ...
  const copy = useCallback(async () => {
    try {
      ...
      /* пусто: подтверждения не будет, и это честнее ложного */
    }
  }, []);

  return (
    <div className="fund-share">
      <span className="fund-share__label">Поделиться</span>

      <button aria-label="Поделиться ВКонтакте" className="fund-share__btn" onClick={() => share("vk")} type="button">
      ...
      <button aria-label="Поделиться в Telegram" className="fund-share__btn" onClick={() => share("tg")} type="button">
        <svg aria-hidden="true" fill="currentColor" height="15" viewBox="0 0 24 24" width="15">
